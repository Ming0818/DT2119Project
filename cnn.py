...
class CNN(Network):
    ...
    def train_model(self, kernel_sizes, dense_size=128):
        ...
        model = Sequential()
        model.add(Conv2D(self.hidden_nodes[0], kernel_size=kernel_sizes[0], strides=(1, 1),
                         activation='relu',
                         input_shape=(self.x_val.shape[1:])))

        for i in range(1, len(self.hidden_nodes)):
            model.add(Conv2D(self.hidden_nodes[i], kernel_sizes=kernel_sizes[i]))
        ...
        return model, self.cnn_params_to_folder(kernel_sizes, dense_size)

    def cnn_params_to_folder(self, kernel_sizes, dense_size) -> str:
